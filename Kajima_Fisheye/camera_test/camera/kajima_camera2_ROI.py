...
def remove_outside_bbox(ROI,dets ) :
    logging.debug("RUN THE TESTING")
    # logging.debug("CREATE POLYLINES WITH bbox :{}".format(str(dets)))
    # logging.debug("CREATE POLYLINES WITH ROI :{}".format(str(ROI)))
    # logging.debug("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX")
    # for point in dets :
    # cv2.polylines(image, [self.ROI], True, color=BGRColor.CYAN, thickness=10)
    # logging.debug("CREATE POLYLINES WITH ROI :{}".format(str(ROI)))
    # logging.debug(ROI)
    # logging.debug(type(ROI))
    # logging.debug(ROI.shape)
    # image1 = cv2.polylines(blank.copy(), [ROI], True, color=(255, 0, 0), thickness=1)
    # logging.debug("CREATE POLYLINES WITH bbox :{}".format(str(int(dets[0]))))
    # logging.debug("CREATE POLYLINES WITH bbox :{}".format(str(int(dets[1]))))
    # logging.debug("CREATE POLYLINES WITH bbox :{}".format(str(int(dets[2]))))
    # logging.debug("CREATE POLYLINES WITH bbox :{}".format(str(int(dets[3]))))
    current_points = [[int(dets[0]),int(dets[1])],[int(dets[0]),int(dets[3])],[int(dets[2]),int(dets[1])],[int(dets[2]),int(dets[3])]]
...
class Camera(Adaptor):
    def __init__(self, args, **kw):
        ...
        self.run()
        self.ROI = np.array(self.cfg.get('cam_roi', [0,0,0,2991,2991,2991,2991,0]), dtype=np.int).reshape(4, -1)
